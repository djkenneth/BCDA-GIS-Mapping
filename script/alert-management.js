...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const alertsInterface = document.getElementById(
    ...
    if (window.matchMedia("(max-width: 768px)").matches) {
      ...
      if (header) {
        if (header.classList.contains("collapsed")) {
          ...
          topPosition = "244px"; // Changed from 191px to 244px
        }
      }
  ...
  function updateSummaryCards(alerts) {
    ...
    const activeCount = alerts.filter(
      (a) => a.status === "normal" || a.status === "active"
    ).length;
  ...
  function updatePaginationInfo() {
    ...
    if (pageStart && pageEnd && totalRecords) {
      const start =
        filteredAlerts.length === 0 ? 0 : (currentPage - 1) * itemsPerPage + 1;
  ...
  function applyFilters() {
    ...
    const filtered = alerts.filter((alert) => {
      ...
      if (
        categoryFilter !== "all" &&
        !alert.category.toLowerCase().includes(categoryFilter)
      ) {
        return false;
      }
      ...
      if (
        locationFilter !== "all" &&
        !alert.location.toLowerCase().includes(locationFilter)
      ) {
        return false;
      }

document.addEventListener("DOMContentLoaded", function () {
  ...
  function setupTriggerListeners() {
    Object.entries(SELECTORS.componentTriggers).forEach(
      ([triggerSelector, componentSelector]) => {
  ...
  function setupComponentCloseButtons() {
    SELECTORS.contentComponents.forEach((selector) => {
      ...
      components.forEach((component) => {
        ...
        const closeButtons = component.querySelectorAll(
          '.close-btn, .close-panel, [id$="-close"], [id$="close-btn"], .card-close-btn'
        );
      ...
      const observer = new MutationObserver(function (mutations) {
        mutations.forEach(function (mutation) {
          if (mutation.type === "childList") {
            mutation.addedNodes.forEach(function (node) {
              if (node.nodeType === 1) {
                const newCheckboxes = node.querySelectorAll(
                  'input[type="checkbox"]'
                );
    ...
    const observer = new MutationObserver(function (mutations) {
      mutations.forEach(function (mutation) {
        if (
          ...
          infoDrawer.classList.contains("open")
        ) {
  ...
  function setupInfoDrawerButtons() {
    ...
    const cancelInspectionBtn = document.querySelector(

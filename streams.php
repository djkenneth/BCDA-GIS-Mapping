...
<head>
    ...
    <!-- Add Picture-in-Picture polyfill to prevent reference errors -->
    <script>
    // Polyfill for documentPictureInPicture
    (function() {
      if (typeof documentPictureInPicture === 'undefined') {
        console.log('Picture-in-Picture API not supported, using polyfill');
        window.documentPictureInPicture = {
          requestWindow: function() {
            return Promise.reject(new Error('Picture-in-Picture API not supported'));
          },
          __isPolyfill: true
        };
      }
...
<?php include("../components/header.php") ?>
    ...
    <script>
        // Configuration
        const MAX_STREAMS = 4;
        // Fix API URL typo - change itbsstudio to itbstudio
        const API_BASE_URL = 'https://onlitower.itbstudio.com'; 
        ...
        async function fetchDevices() {
            try {
                ...
                // Parse the response based on the new specified format
                if (data && data.code === 1000 && data.data && Array.isArray(data.data.pageData)) {
                    devices = data.data.pageData.map(device => ({
                        ...
                        // Format channel ID with $1$0$0 suffix as specified
       ...
       async function startStream(device) {
        ...
        async function stopStream(streamId) {
            try {
                ...
                try {
                    // Try to clean up the stream, but don't let it block the main operation if it fails
                    await fetch(`${API_BASE_URL}/streams/cleanup/${streamId}`, {
                        method: 'POST'
                    });
                } catch (cleanupError) {
                    console.warn('Stream cleanup error:', cleanupError);
                ...
                // Try to update UI anyway to keep UI in sync
                activeStreams = activeStreams.filter(stream => stream.id !== streamId);
                updateStreamGrid();
            }
        }
        ...
        async function clearAllStreams() {
            try {
                ...
                // Even if cleanup fails, proceed with local cleanup
        ...
        function initializeHlsPlayer(streamId, streamUrl) {
            ...
            if (Hls.isSupported()) {
                ...
                // Add timeupdate listener to ensure loader is hidden once video plays
                ...
                if (loaderElement) {
                    loaderElement.innerHTML = `
                        <div>Your browser doesn't support HLS streaming.</div>
                        <div>Click View to open full stream viewer.</div>
                    `;
                }
            }
        }
        ...
        window.addEventListener('error', function(e) {
            ...
            if (target.tagName === 'IMG') {
                ...
                // Check if it's one of the logo images that's failing
                if (src.includes('/logos/') && src.includes('-icon.png')) {
                    console.log('Handling missing image:', src);
                    
                    // Replace with a default image
                    target.src = 'data:image/svg+xml;base64,';
        ...
        document.addEventListener('DOMContentLoaded', function() {
          // Force the header to be expanded when the page loads
          const header = document.querySelector('header');
          if (header) {
            // First remove any collapsed class
            ...
            // Override any existing script behavior
            const existingHeaderScript = document.querySelector('script[src*="header.js"]');
            if (existingHeaderScript) {
              // Create a new observer to monitor the header
              const observer = new MutationObserver(function(mutations) {
                mutations.forEach(function(mutation) {
                  if (mutation.attributeName === 'class' && header.classList.contains('collapsed')) {
                    // If header gets collapsed by another script, expand it again
                    header.classList.remove('collapsed');
                    localStorage.removeItem('headerCollapsed');
                    setTimeout(() => {
                      window.dispatchEvent(new Event('resize'));
    ...
    <!-- Use our custom header script -->
    <script src="../script/header-streams.js?t=<?php echo time(); ?>"></script>
